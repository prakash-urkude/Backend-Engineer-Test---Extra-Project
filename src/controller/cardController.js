...
const createCard = async function (req, res) {
    ...
    try {
        ...
        if (!validators.isValid(data)) return res.status(400).send({ status: false, message: "plese enter data in request body" })
        ...
        let cardTotal = await cardModel.find().count()
        let cardNumber = "c" + (00 + (++cardTotal))
